...
(function($) {
    $.fn.youamaAjaxLogin = function(options) {
        ...
        function openCloseWindowEvents() {
            ...
            $('a[href*="register"], .new-users button')
                .on('click', function() {
                $('.skip-links .skip-account').trigger('click');
                animateCloseWindow('login', false, false);
                animateShowWindow('register');
        ...
        function callAjaxControllerRegistration() {
            ...
            if (opts.stop != true) {
                ...
                var ajaxRegistration = jQuery.ajax({
                    url: '/register', //opts.controllerUrl,
                    type: 'POST',
                    data: {
                    ajax : 'register',
                ...
                // Get data
                ajaxRegistration.done(function(jData) {
                    // If there is error
                    if (jData.status != 1) {
                        setError(jData.errorMsg, 'register');
                    // If everything are OK
                    } else {
                        opts.stop = false;
                        animateCloseWindow('register', false, true);
                        // Redirect
                        if (opts.redirection == '1' || global_state_data.loginData.redirection == '1') {
                            window.location = global_state_data.loginData.profileUrl ? global_state_data.loginData.profileUrl : opts.profileUrl;
                        } else {
            ...
            if (opts.stop != true){
                ...
                var ajaxRegistration = jQuery.ajax({
                    ...
                    ajax : 'login',
                        ...
                        "data[User][password]" : opts.password
                    },
                    dataType: "JSON"
                });
                // Get data
